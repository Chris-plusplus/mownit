...
auto legend_width = 250;
auto legend_height = 160;
auto legend_height_error = 80;
...
int main() {
	for (auto&& dir : {
		"interpolation_images/uniform_2/",
		"interpolation_images/uniform_3/",
		"interpolation_images/error_uniform_2/",
		"interpolation_images/error_uniform_3/",
...
#pragma endregion
		...
		cv::imwrite("interpolation_images/uniform_2/" + std::to_string(nodes_count) + ".png", axes_uniform_2.render(window_height, window_width));
		cv::imwrite("interpolation_images/uniform_3/" + std::to_string(nodes_count) + ".png", axes_uniform_3.render(window_height, window_width));
		cv::imwrite("interpolation_images/error_uniform_2/" + std::to_string(nodes_count) + ".png", axes_error_uniform_2.render(window_height, window_width));
		cv::imwrite("interpolation_images/error_uniform_3/" + std::to_string(nodes_count) + ".png", axes_error_uniform_3.render(window_height, window_width));
	...
	while (false and not nodes_counts.empty() and nodes_counts.back() > 20) { // ograniczenie wykresu
...
#define sum_squared_series(__b, __d, __n) auto&& __b ## _ ## __d ## _ ## __n ## _sum_squared_series = axes_sum_squared.create<plt::Series>( \
	...
	std::cout << cv::imwrite("interpolation_images/max_abs.png", axes_max_abs.render(window_height, window_width)) << '\n';
	std::cout << cv::imwrite("interpolation_images/sum_squared.png", axes_sum_squared.render(window_height, window_width)) << '\n';
