...
int main() {
	for (auto&& dir : {
		"approximation_images/trigonometric/uniform/",
		"approximation_images/trigonometric/error_uniform/",
		"approximation_results/trigonometric/"
		 }) {
	...
	auto max_abs_file = std::ofstream("approximation_results/trigonometric/__max_abs.txt");
	auto max_abs_log_file = std::ofstream("approximation_results/trigonometric/__max_abs_log.txt");
	auto sum_squared_file = std::ofstream("approximation_results/trigonometric/__sum_squared.txt");
	auto sum_squared_log_file = std::ofstream("approximation_results/trigonometric/__sum_squared_log.txt");
	...
	for (auto&& nodes_count : nodes_counts) {
		...
		for (size_t m = 2; m <= (nodes_count - 1) / 2; ++m) {
			...
			if (save_whitelist.contains({m, nodes_count})) {
				result_file = std::ofstream("approximation_results/trigonometric/result_" + std::to_string(m) + '_' + std::to_string(nodes_count) + ".txt");
				result_file <<
					"x\ttrigonometric\n";
			}
#endif
			...
			if (save_whitelist.contains({m, nodes_count})) {
				error_file = std::ofstream("approximation_results/trigonometric/error_" + std::to_string(m) + '_' + std::to_string(nodes_count) + ".txt");
				error_file <<
					"x\tf(x)\ttrigonometric\n";
...
#pragma endregion
			...
			if (save_whitelist.contains({m, nodes_count})) {
				cv::imwrite("approximation_images/trigonometric/uniform/" + std::to_string(m) + '_' + std::to_string(nodes_count) + ".png", axes_uniform.render(window_height, window_width));
				cv::imwrite("approximation_images/trigonometric/error_uniform/" + std::to_string(m) + '_' + std::to_string(nodes_count) + ".png", axes_error_uniform.render(window_height, window_width));
			}
	...
	auto trig_max_abs_dat = std::ofstream("approximation_results/trigonometric/__max_abs.dat");
	auto trig_sum_squared_dat = std::ofstream("approximation_results/trigonometric/__sum_squared.dat");
